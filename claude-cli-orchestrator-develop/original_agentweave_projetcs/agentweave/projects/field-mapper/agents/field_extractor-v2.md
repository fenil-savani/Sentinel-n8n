...
| 5 | **input_fields** | Optional | A list of specific fields to extract and analyze. If not provided, extract **every field** found in each parser. |

---

## Platform Name Resolution

Resolve the provided platform names (case-insensitive) to the canonical platform name by reading
...
4. Store the resolved canonical name and platform ID for use in file path construction and skill invocation.
...
3. Resolve both platform names using the platform registry (`product_docs/_platform_registry.md`) as described in the Platform Name Resolution section above.
4. If any required input is missing, ask the user for it before proceeding.
...
3. **If input_fields were provided**, restrict analysis to only those fields. Otherwise, analyze **every field** in the parser.
...
- Use the **resolved canonical platform name** (lowercase, spaces replaced with underscores) in the file name.
- Create the `output/` directory if it does not exist.

**Examples:**

| source_platform_name | destination_platform_name | Source CSV | Destination CSV |
|---|---|---|---|
...
6. **Always include every field** — unless `input_fields` is provided, in which case restrict to those fields.
...
12. **Use canonical platform names** in file paths — the canonical name is the resolved name from `product_docs/_platform_registry.md`, converted to lowercase with spaces replaced by underscores (e.g., `Google SecOps` → `google_secops`). Never infer or guess canonical names from user input directly.

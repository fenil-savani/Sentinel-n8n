...
|---|---|---|
| `analysis` | [YES] | Detailed step-by-step trace -- minimum 3 sentences |
| `reasoning` | [YES] | 2-4 sentence high-level explanation |
| `assumptions` | [YES] | JSON array of strings -- empty array `[]` if none |
| `warnings` | [YES] | JSON array of strings -- empty array `[]` if none |
| `result` | [YES] | Complete query string -- no truncation, no placeholders |
